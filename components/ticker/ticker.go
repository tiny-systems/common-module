...
func (t *Component) GetInfo() module.ComponentInfo {
	return module.ComponentInfo{
		...
		Tags:        []string{"SDK"},
		Examples: []string{
			// 1. Config-holder pattern: user credentials and settings
			//    live on the ticker and flow forward via context: {{$}}.
			`Config-holder pattern — carry credentials/config forward to downstream nodes.

Place a ticker at the start of the flow and put ALL user configuration
(tokens, channel IDs, namespaces, thresholds) into its context. Downstream
nodes access fields via {{$.context.fieldName}} after the first hop.
    ...
    - alias: cfg
      ...
      settings:
        ...
        context:
          ...
          channelID: C0PLACEHOLDER
          namespace: production
      settings_schema:
        context:
          type: object
          properties:
            slackToken: { type: string, title: Slack Bot Token, secret: true }
            channelID:  { type: string, title: Slack Channel ID }
            namespace:  { type: string, title: Target Namespace }
          required: [slackToken, channelID, namespace]
    - alias: srv
  ...
  edges:
    - from: cfg:out
      ...
      configuration:
        ...
        readTimeout:  60
        writeTimeout: 15
        context: "{{$}}"    # inject whole ticker settings as http_server context`,

			// 2. HTTP keep-alive pattern: ticker-delay-0 starts a server
			//    once and holds it running forever (the server's start
			//    handler blocks until stopped).
			`HTTP server keep-alive pattern — start once and hold.

The ticker's out port is blocking: it waits for the downstream call to
return before the next tick fires. http_server.start blocks for the
lifetime of the server, so a tiny delay makes the ticker fire once and
then sit idle while the server runs indefinitely.

  nodes:
    - alias: tk
      component: ticker
      module: tinysystems/common-module-v0
      settings: { delay: 1000, context: { port: 8080 } }
    - alias: srv
      component: http_server
      module: tinysystems/http-module-v0
  ...
  edges:
    - from: tk:out
      to:   srv:start
      configuration: { autoHostName: true, readTimeout: 60, writeTimeout: 15, context: "{{$}}" }`,
		},
	}
}
